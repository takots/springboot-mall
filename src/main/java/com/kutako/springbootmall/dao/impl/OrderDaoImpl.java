...
import com.kutako.springbootmall.rowmapper.OrderRowMapper;
...
public class OrderDaoImpl implements OrderDao {
    ...
    public List<OrderItem> getOrderItemByOrderId(Integer orderId) {
        String SqlStr=" SELECT oi.order_item_id ,oi.order_id ,oi.product_id ,oi.quantity ,oi.amount ,p.product_name ,p.image_url"
                     +" FROM order_item as oi"
                     +" LEFT JOIN product as p ON oi.product_id = p.product_id WHERE oi.order_id = :orderId";
        Map<String ,Object> map = new HashMap<>();
        map.put("orderId" ,orderId);
        List<OrderItem> orderItemList = namedParameterJdbcTemplate.query(SqlStr ,map ,new OrderItemRowMapper());
        return orderItemList;
    }

    @Override
    public Integer createOrder(Integer userId, Integer totalAmount) {
        String SqlStr="INSERT INTO `order` (user_id ,total_amount ,created_date ,last_modified_date)"
    ...
    public void createOrderItems(Integer orderId, List<OrderItem> orderItemList) {
        // 方法1: 使用 for loop 一條一條 sql 加入數據 ,效率低
//        for(OrderItem orderItem : orderItemList){
//            String SqlStr="INSERT INTO order_item (order_id ,product_id ,quantity ,amount)"
//                         +"VALUES (:orderId ,:productId ,:quantity ,:amount)";
//            Map<String ,Object> map = new HashMap<>();
//            map.put("orderIt" ,orderId);
